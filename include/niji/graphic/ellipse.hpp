...
namespace niji
{
    ...
    struct ellipse
    {
        ...
        point_type center;
        niji::radius<T> radius;

        ellipse(point_type const& pt, niji::radius<T> const& radius)
        ...
        {
            iterate_impl(sink, radius.x, radius.y);
        }
    ...
    private:
        template<class Sink>
        void iterate_impl(Sink& sink, T rx, T ry) const
        {
...
#   if defined(NIJI_NO_CUBIC_APPROX)
            T sx = rx * numbers::tan_pi_over_8<T>,
              sy = ry * numbers::tan_pi_over_8<T>,
              mx = rx * numbers::root2_over_2<T>,
              my = ry * numbers::root2_over_2<T>,
...
#   else
            T sx = rx * numbers::cubic_arc_factor<T>,
              sy = ry * numbers::cubic_arc_factor<T>,
